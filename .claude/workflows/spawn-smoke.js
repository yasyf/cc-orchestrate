...
// the whole live loop: build the binary, select tmux, create a temp project, spawn
// an agent, assert status populates + send-message delivers + kill cleans up, then
// tears everything down. Returns the observed agent status.
//
// Run with: /spawn-smoke

export const meta = {
  name: 'spawn-smoke',
  description: 'End-to-end spawn smoke on the tmux backend: build, spawn, assert status + send-message + kill, then clean up',
  phases: [{ title: 'Smoke', detail: 'live build → spawn → status → send-message → kill → teardown' }],
}
...
const SCHEMA = {
  ...
  required: ['pass', 'built', 'tmuxAvailable', 'spawned', 'statusPopulated', 'messageDelivered', 'killedClean', 'status', 'notes'],
  properties: {
...
3. TEMP PROJECT: make an isolated workdir \`PRJ=$(mktemp -d)\` and \`git init\` in it (backends may assume a repo). Create the project:
   \`$BIN projects create smoke-$$ --backend tmux --cwd "$PRJ"\`. Capture the printed project id. Confirm a detached tmux session now exists (\`tmux list-sessions\`).

4. SPAWN: \`$BIN agent spawn --project <id> --name smoke --cwd "$PRJ" --prompt "say hello then wait"\`. Capture the agent id, backend, and terminal (a tmux pane id). Set spawned from whether all three are present.

5. STATUS: poll \`$BIN agent status <agent-id>\` every ~2s for up to ~30s. Record the LAST output verbatim into "status". Set statusPopulated=true once "state" is non-empty (e.g. running/active). NOTE: the derived status is tailed from ~/.claude (or \$CLAUDE_CONFIG_DIR)/projects/**/<session>.jsonl — it only fully populates if \`claude\` actually runs in the pane. If \`claude\` is not installed so no transcript ever appears, set statusPopulated=false and say so in notes; do NOT hang past the timeout.

6. SEND-MESSAGE: \`$BIN agent send-message <agent-id> "ping"\`. Set messageDelivered=true if it prints a seq and exits 0.

7. KILL + CLEANUP: \`$BIN agent kill <agent-id>\`, then \`$BIN agent list --project <id>\` and confirm the agent is gone, and \`tmux list-panes -s -t <session>\` (or list-windows) confirms the agent's window/pane is closed. Set killedClean accordingly. Finally tear down: \`tmux kill-session -t <session>\` and \`rm -rf "$PRJ" /tmp/cc-orchestrate-smoke\`. Leave no smoke-* tmux sessions or temp dirs behind.

Set pass=true only if built, spawned, messageDelivered, and killedClean are all true (statusPopulated may be false when claude is unavailable — record that in notes). Return the schema.`
