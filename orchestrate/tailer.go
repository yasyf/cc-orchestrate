...
// pollInterval is the cadence at which the tailer polls for the transcript to
...
// onStatus error. scope is currently informational. The baseline status is the
// empty accumulator's StateUnknown, which matches the agents table default, so the
// first emission is the first meaningful change rather than a redundant unknown.
func runTailer(ctx context.Context, sessionID, scope string, onStatus func(Status) error) error {
	path, ok := waitForTranscript(ctx, sessionID)
	if !ok {
		return nil
	}
...
func waitForTranscript(ctx context.Context, sessionID string) (string, bool) {
