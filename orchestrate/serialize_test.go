...
func TestSerializeRestoreRoundTrip(t *testing.T) {
	...
	serLog := &eventLog{}
	reply := handleSerialize(opCtx(db, mustJSON(t, map[string]string{"out": out}), serLog.append))
	if !reply.OK {
	...
	resLog := &eventLog{}
	rReply := handleRestore(opCtx(db, mustJSON(t, map[string]string{"path": out}), resLog.append))
	if !rReply.OK {
...
func TestRestoreIntoWipedDB(t *testing.T) {
	...
	out := filepath.Join(t.TempDir(), "bundle.json")
	if reply := handleSerialize(opCtx(db, mustJSON(t, map[string]string{"out": out}), (&eventLog{}).append)); !reply.OK {
		t.Fatalf("serialize failed: %s", reply.Error)
	}
	...
	log := &eventLog{}
	reply := handleRestore(opCtx(db, mustJSON(t, map[string]string{"path": out}), log.append))
	if !reply.OK {
...
func TestSerializeWriteFailureAppendsNoEvent(t *testing.T) {
	...
	log := &eventLog{}
	reply := handleSerialize(opCtx(db, mustJSON(t, map[string]string{"out": out}), log.append))
	if reply.OK {
...
func TestRestoreStrictParseFailsLoud(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			log := &eventLog{}
			reply := handleRestore(opCtx(db, mustJSON(t, map[string]string{"path": path}), log.append))
			if reply.OK {
