...
func appDaemonRole() daemonrole.Classifier {
	role := daemonrole.Classifier{
		RoleID: "com.yasyf.cc-orchestrate", RolePath: filepath.Join(appPaths().StateDir(), "bin", AppName),
	}
	if err := provisionDaemonRole(role.RolePath, buildVersion()); err != nil {
		panic(err)
	}
	return role
}

func provisionDaemonRole(rolePath, callerVersion string) error {
	...
	if target, targetErr := filepath.EvalSymlinks(rolePath); targetErr == nil {
		...
		if target == executable || (versionErr == nil && !version.Newer(callerVersion, currentVersion)) {
			return nil
		}
	}
	...
	tmp, err := os.CreateTemp(filepath.Dir(rolePath), ".cc-orchestrate-role-*")
	if err != nil {
		return err
	}
	tmpPath := tmp.Name()
...
func executableVersion(path string) (string, error) {
	...
	case <-timer.C:
		_ = cmd.Process.Kill()
		<-done
		return "", errors.New("cc-orchestrate daemon role version timed out")
	}
