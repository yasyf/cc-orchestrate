...
Orchestrate fleets of Claude Code agents across pluggable backends like cmux and superset. Published to PyPI as `cc-orchestrate`; the CLI is `cc-orchestrate`, run as `uvx cc-orchestrate`.

## Repository Structure

```
cc-orchestrate/
├── cc_orchestrate/   # The package — Click CLI and the backend registry
├── tests/            # Pytest suite
├── docs/             # Brand assets; narrative doc pages land here
