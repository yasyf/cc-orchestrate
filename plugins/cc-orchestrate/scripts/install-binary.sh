#!/bin/bash
# GENERATED by cc-guides from .claude/fragments/plugins/cc-orchestrate/scripts/install-binary.sh/ — do not edit; edit the fragments and push — CI renders.
# Locate the binrun runner and exec the cco descriptor.
#
# This renders to plugin scripts/install-binary.sh — the successor to the
# provision-a-symlink installer. bin/cco is a committed symlink to it, so
...
# resolves and execs the version-exact artifact the sidecar bin/cco.binrun
...
if [ -e "$ROOT/.orphaned_at" ] && is_version "${ROOT##*/}"; then
  ...
  for sibling in "${ROOT%/*}"/*/; do
    ...
    [ ! -e "$sibling/.orphaned_at" ] && [ -x "$sibling/bin/cco" ] || continue
    if version_gt "${sibling##*/}" "${newest##*/}"; then
      newest=$sibling
    fi
  done
  [ "$newest" = "$ROOT" ] || exec "$newest/bin/cco" "$@"
fi

DESCRIPTOR="$ROOT/bin/cco.binrun"
